...
/**
 * @name RichPresence
 * @description AutoStartRichPresence.
 * @author Vald
 * @authorId 637417503425036289
 * @version 1.0.0
 * @invite 4GWCMkFTgv
 * @donate https://paypal.me/vald
    ...
    changelog: [
        {
            ...
            type: "fixé",
            item: [
                "Aucun profil active, switch sur le premier.",
                "Si tu veux changer de profil, change manuellement activeProfilID.",
                "sélécteur de profil cassé, en attente de la réparation de ZeresPluginLibrary."
            ]
        }
    ]
};
...
class AutoStartRichPresence {
	...
	async start(){
		if(typeof window.ZeresPluginLibrary === 'undefined'){
			try {
				...
				console.error(e);
				return BdApi.showToast('AutoStartRichPresence: "ZeresPluginLibrary" est introuvable, ou le téléchargement a échoué. le plugin ne peut pas démarrer.', {type: 'error'});
			}
		}
		this.initialize();
	}
	initialize(){
		...
		BdApi.showToast("AutoStartRichPresence à démarré.");
	...
	async stop(){
		...
		BdApi.showToast("AutoStartRichPresence s'arrête.");
	}
	get activeProfile(){
	...
	async _startRichPresence(){
		...
		this.client?.removeAllListerners?.();
		...
		this.client.on("loginFailed", async e => {
			...
			this.client?.removeAllListerners?.();
			this.client?.disconnect?.();
			BdApi.showToast("Authentification ClientID échoué, veuillez vérifier le ClientID.", {type: "error"});
		});
	...
	_updateRichPresence(){
		...
		if(this.activeProfile.button1Label && this.activeProfile.button1URL){
			if(validButtonURLRegex.test(this.activeProfile.button1URL)){
				...
				BdApi.showToast("Bouton 1 URL invalide.", {type: "error"});
			}
		}
		if(this.activeProfile.button2Label && this.activeProfile.button2URL){
			if(validButtonURLRegex.test(this.activeProfile.button2URL)){
				...
				BdApi.showToast("Bouton 2 URL invalide")
			}
		}
	...
	async _startRichPresenceInjection(){
		...
		try{
			...
			if (e.message === "Invalid Client ID"){
				return BdApi.showToast("RichPresence ID est invalide, vérifiez le ClientID.", {type: 'error'});
			}else{
				return BdApi.showToast("Echec de la mise en place du RichPresence.", {type: "error"});
			}
		}
	...
	buildActivityObject(){
		...
		if(this.activeProfile.details){
			activityObject.args.acitivty.details = this.activeProfile.details;
		}
		if(this.activeProfile.state){
			activityObject.args.acitivty.state = this.activeProfile.state;
		}
		if(this.activeProfile.enableStartTime){
			activityObject.args.acitivty.timestamps.start = Math.floor(this.startTime / 1000) * 1000;
		}
		...
		if(this.activeProfile.button1Label && this.activeProfile.button1URL){
			if(this.activeProfile.button1Label.length > 32){
				BdApi.showToast("Bouton 1 ne doit pas dépasser 32 caractères.", {type: 'error'});
			}else if(validButtonURLRegex.test(this.activeProfile.button1Label)) {
				activityObject.args.activity.buttons.push({
					label: this.activeProfile.button1Label,
					url: this.activeProfile.button1URL
				});
			} else{
				BdApi.showToast("URL du bouton 1 est invalide", {type: "error"});
			}
		}
		if(this.activeProfile.button2Label && this.activeProfile.button2URL){
			if(this.activeProfile.button1Label.length > 32){
				BdApi.showToast("Bouton 2 ne doit pas dépasser 32 caractères.", {type: 'error'});
			}else if(validButtonURLRegex.test(this.activeProfile.button2Label)) {
				activityObject.args.activity.buttons.push({
					label: this.activeProfile.button1Label,
					url: this.activeProfile.button1URL
				});
			} else{
				BdApi.showToast("URL du bouton 2 est invalide", {type: "error"});
			}
		}
	...
	generateSettings(panel){
		...
		const profileInputs = new Map([
			...
			["clientID", new window.ZeresPluginLibrary.Settings.Textbox("Client ID", "Client ID de l'application Discord RichPresence", this.profiles[this.session.editingProfile]?.clientID  || "", val => {this.profiles[this.session.editingProfile].clientID = val?.trim?.();}, {disabled: !this.profiles[this.session.editingProfile]})],
			["details", new window.ZeresPluginLibrary.Settings.Textbox("Détails", "La ligne qui vient après le nom du jeu.", this.profiles[this.session.editingProfile]?.details || "", val => {this.profiles[this.session.editingProfile].details = val;}, {disabled: !this.profiles[this.session.editingProfile]})],
			["state", new window.ZeresPluginLibrary.Settings.Textbox("Status", "La ligne qui vient après les détails", this.profiles[this.session.editingProfile]?.state || "", val => {this.profiles[this.session.editingProfile].state = val;}, {disabled: !this.session.profiles[this.session.editingProfile]})],
			["largeImageKey", new window.ZeresPluginLibrary.Settings.Textbox("Large Image", "Grande image.", this.profiles[this.session.editingProfile]?.largeImageKey || "", val => {this.profiles[this.session.editingProfile].largeImageKey = val?.trim?.();}, {disabled: !this.profiles[this.session.editingProfile]})],
			["largeImageText", new window.ZeresPluginLibrary.Settings.Textbox("Large Image Texte", "Texte de la grande image.", this.profiles[this.session.editingProfile]?.largeImageText || "", val => {this.profiles[this.session.editingProfile].largeImageText = val;}, {disabled: !this.profiles[this.session.editingProfile]})],
      		["smallImageKey", new window.ZeresPluginLibrary.Settings.Textbox("Petite Image", "Petite Image.", this.profiles[this.session.editingProfile]?.smallImageKey || "", val => {this.profiles[this.session.editingProfile].smallImageKey = val?.trim?.();}, {disabled: !this.profiles[this.session.editingProfile]})],
      		["smallImageText", new window.ZeresPluginLibrary.Settings.Textbox("Petite Image texte", "Texte de la petite image.", this.profiles[this.session.editingProfile]?.smallImageText || "", val => {this.profiles[this.session.editingProfile].smallImageText = val;}, {disabled: !this.profiles[this.session.editingProfile]})],
      		["enableStartTime", new window.ZeresPluginLibrary.Settings.Switch("Temps", "Affiche depuis combien de temps le RichPresence est activé.", this.profiles[this.session.editingProfile]?.enableStartTime, val => {this.profiles[this.session.editingProfile].enableStartTime = val;}, {disabled: !this.profiles[this.session.editingProfile]})],
      		["button1Label", new window.ZeresPluginLibrary.Settings.Textbox("Bouton 1 Label", "Label pour le bouton", this.profiles[this.session.editingProfile]?.button1Label || "", val => {this.profiles[this.session.editingProfile].button1Label = val;}, {disabled: !this.profiles[this.session.editingProfile]})],
      		["button1URL", new window.ZeresPluginLibrary.Settings.Textbox("Bouton 1 URL", "URL pour le bouton.", this.profiles[this.session.editingProfile]?.button1URL || "", val => {this.profiles[this.session.editingProfile].button1URL = val?.trim?.();}, {disabled: !this.profiles[this.session.editingProfile]})],
      		["button2Label", new window.ZeresPluginLibrary.Settings.Textbox("Bouton 2 Label", "Label pour le bouton.", this.profiles[this.session.editingProfile]?.button2Label || "", val => {this.profiles[this.session.editingProfile].button2Label = val;}, {disabled: !this.profiles[this.session.editingProfile]})],
      		["button2URL", new window.ZeresPluginLibrary.Settings.Textbox("Bouton 2 URL", "URL pour le bouton.", this.profiles[this.session.editingProfile]?.button2URL || "", val => {this.profiles[this.session.editingProfile].button2URL = val?.trim?.();}, {disabled: !this.profiles[this.session.editingProfile]})]
		]);
		...
		const createActiveProfileDropdown = () => {
			return new window.ZeresPluginLibrary.Settings.Dropdown("Selection d'un profil actif", "Switch de profil actuellement cassé; modifier activeProfilID.", this.settings.activeProfileID, this.profiles.map((p, id) => ({label: p.name, value: id})), val => {this.settings.activeProfileID = val;}, {disabled: !this.profiles.length});
		};

		const createRPCInjectionSwitch = () => {
			return new window.ZeresPluginLibrary.Settings.Switch("RPC Event Injection", "Bypass l'utilisation d'IPC prévient les autres programmes de ne pas utiliser leur propres RichPresence. Si il y'a des erreurs, mettez le switch en OFF.", this.settings.rpcEventInjection, val => {
		...
		newProfileButton.innerText = "Création de Profil";
		...
		deleteProfileButton.innerText = "Supression du Profil";
		...
		deleteProfileButton.addEventListener("click", e => {
			...
			BdApi.showConfirmationModal("Suppresion du profil RichPresence", `Es-tu sûr de vouloir supprimer le profil ${this.profiles[profileIDToDelete].name} ?`, {
				danger: true,
				confirmText: "Delete",
				onConfirm: () => {
		...
		const editProfileGroup = new window.ZeresPluginLibrary.Settings.SettingGroup("Séléctionner le profil à éditer", {collapsible: false, shown: true}).appendTo(panel).append(editProfileDropdown, deleteProfileButton);
		reloadEditProfileGroup = () => {
		...
		new window.ZeresPluginLibrary.Settings.SettingGroup("Edition du Profil Actif", {collapsible: false, shown: true, callback: () => {if (this.session.editingProfile === this.session.settings.activeProfileID) {clearTimeout(this.currentTimeout); this.currentTimeout = setTimeout(() => {this.updateSettings(); this.updateProfiles(); this.updateRichPresence();}, 5000);}}}).appendTo(panel).append(...profileInputs.values());
		let div = document.createElement("div");
		div.innerHTML = '<a href="https://discordapp.com/developers/applications/me" rel ="noreferrer noopener" target="_blank">Crée ou Modifier votre application Discord RichPresence.</a>';
	...
	askToDownloadZeresPluginLibrary(){
		return new Promise((resolve, reject) => {
			BdApi.showConfirmationModal("Bibliothèque manquante", `La bibliothèque à besoin du plugin ${this.constructor.name}. Cliquez pour l'installer.`, {
